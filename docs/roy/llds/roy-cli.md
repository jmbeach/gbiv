...
The `roy` binary serves two roles from a single executable:

1. **Daemon mode** (`roy start`): runs the HTTP Server in the foreground.
2. **Client mode** (`roy status`, `roy get`, `roy send`): thin HTTP clients that talk to a running daemon.

Splitting these into two binaries was considered and rejected — sharing one binary keeps installation simple (`cargo install roy`) and lets the skill teach a single command name. clap's subcommand groups handle the dispatch.

### Audience

**roy is designed first and foremost as a tool for an LLM (specifically, a Claude Code session loaded with the roy skill).** Every CLI design choice in this document — JSON-only output, structured error bodies, verbose `explanation` fields on guard rejections, distinct exit codes, no pretty-printing — is in service of that consumer. Human use (a developer typing `roy status` in a terminal) is *supported* but not optimized for; the recommended human path is to pipe the JSON through `jq`. If a future change forces a tradeoff between LLM-friendliness and human-friendliness, LLM wins.

## Subcommands

```
roy start         [--session-name <NAME>] [--bind <ADDR>]
roy status        [--lines <N>]
roy get <COLOR>   [--lines <N> | --start-line <N> --end-line <N>]
roy send <COLOR> <TEXT>
roy install-skill [--scope user|project] [--force]
```

`roy status` and `roy get` always print JSON to stdout — there is no human-pretty mode. See "Output Format" below for the rationale.

No `roy stop` — Ctrl+C in the daemon's terminal is the only stop mechanism in v1 (HLD).
No `roy restart` — that's `Ctrl+C` then `roy start` again.
...
Prints the JSON response from `GET /sessions` to stdout — one entry per color with `pane_status`, `claude_pane`, the last N lines of pane output, and the truncation/range fields. roy does **not** classify the session ("idle", "waiting", "building"); the consuming Claude reads the raw lines and decides.
...
Calls `GET /session/:color` and prints the JSON response to stdout — same `output_truncated`, `output_original_bytes`, `output_returned_bytes`, `range_returned` fields as the API. No human-pretty mode for the same reason as `roy status` (single surface, JSON is the contract).
...
```
roy send red "please run the tests"
roy send red "let me know when you're done"
```
...
Writes the bundled skill (`SKILL.md` and any future companion files) into the user's Claude Code skills directory so a Claude Code session can discover and load it. The skill source is `include_str!`'d (or `include_dir!`'d) into the `roy` binary at compile time so a `cargo install`'d copy with no nearby source tree still works.

Flags:
- `--scope user` (default): writes to `~/.claude/skills/roy/`. Available across every Claude Code project on this machine.
- `--scope project`: writes to `<gbiv-root>/.claude/skills/roy/`. Available only inside this gbiv workspace; useful when the user is testing a custom skill or wants per-project pinning.
...
{
  "scope": "user",
  "destination": "/Users/jane/.claude/skills/roy/SKILL.md",
...
The `version:` line is added to the skill frontmatter as part of this design (see roy-skill LLD § "Versioning") so install-skill can reason about updates without parsing the body.
...
2. Read `<gbiv-root>/main/<repo>/.roy/port`. If missing, exit 2 with `"daemon not running (no port file at <path>); start it with: roy start"`.
3. Parse as a `u16`. If malformed, exit 2 with `"port file at <path> is corrupt"`.
4. Open a TCP connection to `127.0.0.1:<port>`. If `ECONNREFUSED`, exit 2 with `"port file present but daemon not responding (stale?); restart with: roy start"`.

Stale port files are detected lazily — on the next `roy start`, the new daemon overwrites the file with its real port.
...
```
roy: <subcommand>: <human-readable message>
...
Successful JSON output goes to stdout; on error the command writes nothing to stdout and prints the error chain to stderr. This means a script can run `roy status` and rely on stdout being either valid JSON or empty.

The CLI itself depends on `anyhow` only; any reusable helpers it shares with the daemon (port file resolution, etc.) live in `gbiv-core` and use `thiserror`-typed errors.

## Logging

roy uses a structured logging framework from day one — even though v1 only writes to stderr, having levels and structured fields in place avoids a painful retrofit when file/syslog sinks land later.
...
1. **`RUST_LOG` env var** — full `EnvFilter` syntax (e.g., `RUST_LOG=roy=debug,tiny_http=warn`). If set, takes precedence over flags. Standard Rust convention; lets power users tune per-module without touching the CLI surface.
...
```
2026-04-28T13:45:01.234Z  INFO roy::server: listening on http://127.0.0.1:54321
2026-04-28T13:45:03.118Z DEBUG roy::pane_locator: resolving color=red window_id=@7 panes=3
2026-04-28T13:45:03.142Z  INFO roy::server: GET /session/red 200 24ms
```
...
Both `roy start` (daemon) and the client subcommands install the same subscriber at process start, before any other work. The init helper lives in `gbiv-core` (or a small shared module) so the daemon and CLI cannot drift on log format.
...
|---|---|---|---|
| Single binary, multiple subcommands | Yes | Separate `royd` daemon binary | One install, one command for the skill to teach |
...
| `roy status` output format | JSON only | Pretty table; header + raw lines; heuristic-classified state ("idle"/"waiting"/etc.) | LLM-first audience (see § Context > Audience). Classification belongs in the LLM, not in roy |
| Default `--lines` for `roy status` | 35 | 5; 50; match `roy get` (200) | Big enough to capture a prompt-with-options or tool-use exchange; small enough that a 7-color survey doesn't blow context |
...
| Port file path | `<main-worktree>/.roy/port` | `~/.roy/<hash>.port`, `$XDG_STATE_HOME` | HLD decision: in-workspace; `roy start` auto-adds `.roy/` to `.git/info/exclude` via `gbiv-core::ensure_gitignore_entry` so the user never edits gitignore |
...
| Daemon's tmux session was killed mid-session | `roy status` returns 200 with all colors `no_window` (since `list_windows` will fail to find the session, daemon returns 503; CLI surfaces "tmux session not found"). v1 does not auto-recover |
| `roy send red ""` | Local validation: exit 1 with "text must not be empty" before opening a connection |
| `roy send red <very long text>` | Forwarded as-is; HTTP server enforces any size limits, not the CLI |
| Two `roy send red` racing | Both fire HTTP requests; daemon serializes (HTTP Server: thread-per-request, tmux serializes keys per-pane) |
| `--lines` combined with `--start-line`/`--end-line` on `roy get` | clap rejects with a usage error before any HTTP call |
| User invokes `roy start` while a daemon is already running | TCP bind fails; exit 1 with "another roy daemon may be running on port <N>; check the port file or run `lsof -i:<N>`" |

## Technical Debt & Future Work

1. **No `roy stop`.** Foreground-only is the v1 contract. If `--detach` ever ships, `stop` ships with it.
...
- HLD: `docs/roy/high-level-design.md` § Components > roy CLI
- Companion: `docs/roy/llds/http-server.md` (server side of the HTTP)
- Companion: `docs/roy/llds/roy-skill.md` (primary consumer)
