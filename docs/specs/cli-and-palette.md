...
- [x] CLI-COLOR-015: When `is_valid_color` is called with a name that matches an entry in the COLORS array, the system shall return true.
- [x] CLI-COLOR-016: When `is_valid_color` is called with a name that does not match any entry in the COLORS array, the system shall return false.
