...
- [x] TMX-SESSION-006: When enumerating worktree paths, the system shall consider "main" first, followed by the active palette in canonical order (the seven base ROYGBIV colors red, orange, yellow, green, blue, indigo, violet, then any configured extras in declared order).
...
- [x] TMX-SYNC-007: When parsing GBIV.md, the system shall extract the set of active colors by collecting tags that are names in the active palette, deduplicating them.
- [x] TMX-SYNC-008: When an active-palette name is active in GBIV.md but has no corresponding tmux window, the system shall identify it as a missing window.
...
- [x] TMX-SYNC-013: When sorting windows, the system shall place "main" first, then active-palette names in canonical order (base ROYGBIV — red, orange, yellow, green, blue, indigo, violet — then configured extras in declared order), then windows not in the active palette preserving their relative order.
...
- [x] TMX-CLEAN-005: When a window name is a name in the active palette AND is not present as any tag in GBIV.md, the system shall classify that window as orphaned.
...
- [x] TMX-CLEAN-007: When a window name is not a name in the active palette (e.g., "bash", "htop"), the system shall never classify it as orphaned.
