...
struct WorktreeStatus {
    ...
    merged: Option<bool>,
    age: Option<Duration>,
    ahead_behind: Option<Result<(u32, u32), ()>>,
}
    ...
    for (i, result) in results.into_iter().enumerate() {
        ...
        match result {
            ...
            Some(status) => {
                ...
                let dirty_str = if status.is_dirty { "dirty" } else { "clean" };

                if branch == color {
                    println!("{}{:<8}{}  {:<24} {}", color_code, color, RESET, branch, dirty_str);
                } else {
                    let merged_str = match status.merged {
                        Some(true) => "merged",
                        Some(false) => "not merged",
                        None => "no remote",
                    };
                    let age_str = status.age.map(format_age).unwrap_or_else(|| "???".to_string());
                    let ab_str = match status.ahead_behind {
                        Some(Ok((ahead, behind))) => format!("↑{} ↓{}", ahead, behind),
                        Some(Err(())) => "no upstream".to_string(),
                        None => "???".to_string(),
                    };
                    println!(
                        "{}{:<8}{}  {:<24} {}  {}  {}  {}",
                        color_code, color, RESET, branch, dirty_str, merged_str, age_str, ab_str
                    );
                }
            }
        }
